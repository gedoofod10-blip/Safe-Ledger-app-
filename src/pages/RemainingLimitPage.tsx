              ...
              <CardContent className="space-y-3">
                <div className="relative w-36 h-36 mx-auto">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className={`text-xl font-bold ${isOver ? 'text-debit' : 'text-credit'}`}>{Math.abs(cl.remaining).toLocaleString()}</span>
                    <span className="text-xs text-muted-foreground">متبقي</span>
                  </div>
                </div>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between p-2 bg-muted rounded-lg">
                    <span>السقف</span><span className="font-bold">{cl.client.budgetLimit!.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between p-2 bg-muted rounded-lg">
                    <span className="flex items-center gap-1">عليه <ArrowDown className="w-3 h-3 text-debit" /></span>
                    <span className="font-bold text-debit">{cl.debit.toLocaleString()}</span>
                  </div>
                  <div className="flex justify-between p-2 bg-muted rounded-lg">
                    <span className="flex items-center gap-1">له <ArrowUp className="w-3 h-3 text-credit" /></span>
                    <span className="font-bold text-credit">{cl.credit.toLocaleString()}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
